        ...
        return client.fetch(ConfigMap.class, request.name())
            .map(configMap -> {
                ...
                if (data.containsKey("basic")) {
                    ...
                    try {
                        var opensearchProperties =
                            JsonUtils.mapper().readValue(json, OpensearchProperties.class);
                        eventPublisher.publishEvent(
                            new ConfigUpdatedEvent(this, opensearchProperties)
                        );
                    } catch (JsonProcessingException e) {
                        log.warn("Failed to parse opensearch properties", e);
                    }
                }
                return Result.doNotRetry();
            })
    ...
    public Controller setupWith(ControllerBuilder builder) {
        ...
        var matcher = DefaultExtensionMatcher.builder(client, configMap.groupVersionKind())
            .fieldSelector(FieldSelector.of(
                QueryFactory.equal("metadata.name", "opensearch-engine-config")))
            .build();
        return builder
            .extension(configMap)
